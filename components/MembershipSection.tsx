...
const MembershipSection: React.FC<MembershipSectionProps> = ({ onTierSelect }) => {
  ...
  // Extract elite passcode from URL and store in sessionStorage
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const elitePasscode = urlParams.get('elitePasscode');
    
    if (elitePasscode) {
      sessionStorage.setItem('referralCode', elitePasscode);
      // Clean up URL by removing the elitePasscode parameter
      const newUrl = new URL(window.location.href);
      newUrl.searchParams.delete('elitePasscode');
