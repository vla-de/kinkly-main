...
const PricingTier: React.FC<PricingTierProps> = ({ title, price, description, features, isFeatured = false, onSelect, ctaText, remainingTickets, locked = false }) => {
  ...
  useEffect(() => {
    // Check if tickets decreased (purchase happened)
    if (previousTickets !== null && remainingTickets !== undefined && remainingTickets < previousTickets) {
      setShowPurchaseAnimation(true);
      // Animation sequence: grow -> explode -> settle
      setTimeout(() => setShowPurchaseAnimation(false), 2000);
    }
    setPreviousTickets(remainingTickets || null);
  }, [remainingTickets, previousTickets]);
                ...
                <span 
                  className={`inline-block transition-all duration-1000 ${
                    showPurchaseAnimation 
                      ? 'text-4xl font-black text-red-500 animate-ping' 
